@if (count($comments) > 0)
 @foreach ($comments as $comment)
        <div class="media" style="border-bottom: 0;margin-top: 0;padding-bottom: 15px;position: relative;">
            ...
            ">
                ...
                <button type="button" data-toggle="reply-form" data-target="comment-{{$comment->id}}-reply-form" class="btn btn-info btn-link " ><i class="fa fa-reply"></i> Reply</button>
                <form action="{{ route('comment.reply') }}" method="POST" class="reply-form d-none" id="comment-{{$comment->id}}-reply-form" style="padding: 10px 0 30px 0">
                    @csrf
                    ...
                    <div class="media-footer">
                       ...
                       <button type="button" data-toggle="reply-form" data-target="comment-1-reply-form" class="btn btn-danger pull-right">Cancel</button>
                ...
                @if (Auth::user()->id == $comment->getUser->id)
                    <div class="action d-flex pull-right align-self-end " style="position: absolute;top: 0;right: 0;">
                        ...
                        <form action="{{ route('comment.destroy', $comment->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-just-icon"><i class="fa fa-trash"
